...
# -----------------------------
# Scraper básico de Google News
# -----------------------------
def scrape_gnews(query, lang="es", country="AR", max_results=5):
    url = f"https://news.google.com/search?q={query}&hl={lang}&gl={country}&ceid={country}:{lang}"
    headers = {"User-Agent": "Mozilla/5.0"}
    r = requests.get(url, headers=headers, timeout=10)
    soup = BeautifulSoup(r.text, "html.parser")
    articles = []
    for a in soup.select("a.DY5T1d")[:max_results]:
        link = a["href"]
        if link.startswith("./"):
            link = "https://news.google.com" + link[1:]
        title = a.get_text(strip=True)
...
# -----------------------------
# Modelo de resumen liviano
# -----------------------------
summarizer_es = pipeline("summarization", model="mrm8488/t5-small-finetuned-summarization-es", device=-1)
...
# Interfaz Streamlit
# -----------------------------
st.set_page_config(page_title="Noticias resumidas", page_icon="📰")
st.title("📰 Buscador de noticias con resumen")

query = st.text_input("🔍 Buscar noticias sobre:", "Formula 1")
num_articles = st.slider("Número de artículos", 1, 10, 5)

if st.button("Buscar"):
    st.write("Buscando noticias...")
    articles = scrape_gnews(query, max_results=num_articles)
    if articles:
        for a in articles:
            st.markdown(f"**{a['title']}**")
            st.write(f"🔗 [Leer artículo]({a['url']})")
            st.write("📝 Resumen: (simulado)")
            st.write(summarize_text(a['title']))  # usamos el título como texto simulado
            st.write("---")
    else:
        st.warning("No se encontraron artículos.")