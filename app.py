...
def scrape_gnews(query, lang="en", country="US", max_results=5, date_filter=None):
    ...
    if date_filter: query = f"{query} when:{date_filter}" 
    ...
    soup = BeautifulSoup(r.text, "html.parser") # Extract titles + links 
    articles = [] 
...
def send_email(subject, body, recipients): 
    ...
    data = { 'Messages': [ { "From": {"Email": "cuenta.de.repuesto2021@gmail.com", "Name": "News Bot"}, 
...
date_filter = st.selectbox( "Filtrar por fecha", options=["", "1d", "7d", "1m"], format_func=lambda x: { "": "Sin filtro", "1d": "Últimas 24 horas", "7d": "Última semana", "1m": "Último mes" }[x] ) 
...
if st.button("Buscar y enviar"): 
    ...
    if articles: 
        ...
        if status == 200: st.success("✅ Email enviado!")
        else: 
